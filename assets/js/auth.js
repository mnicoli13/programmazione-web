...
function initAuthState() {
  // Check if the user is logged in via session (server-side)
  $.ajax({
    url: "../api/auth/check_auth.php",
    type: "GET",
    dataType: "json",
    success: function (response) {
      if (response.authenticated) {
        // User is authenticated via session
        setUserMenuState(response.user);
      } else {
        ...
        if (storedUser) {
          ...
          $.ajax({
            ...
            success: function (response) {
              if (response.success) {
                ...
                // Auto login failed, clear localStorage
                localStorage.removeItem("user");
                checkProtectedPage();
              }
            },
          ...
          // No stored user, check if current page is protected
          checkProtectedPage();
        }
      }
    },
...
// Check if current page is protected and redirect if necessary
function checkProtectedPage() {
  ...
  if (currentPath.indexOf(loginPage) === -1) {
    // Check if current page is protected
    $.ajax({
      url: "../api/auth/is_protected.php",
      type: "POST",
      data: { path: currentPath },
      dataType: "json",
      success: function (response) {
        if (response.protected && !response.authenticated) {
          window.location.href = "login.php";
        }
      },
    });
